...
let z = '';
async function test() {
	const infoUrl = `https://msks.vercel.app/movies/dramacool/info?id=${seriesID}`;
...
document.addEventListener("click", async function (event) {
	if (event.target.classList.contains("watch-button")) {
		...
		const watchUrl = `https://msks.vercel.app/movies/dramacool/watch?episodeId=${epiID}&mediaId=${seriesID}`;
		try {
